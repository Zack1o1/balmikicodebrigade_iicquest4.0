...
import { Building2, Users, FileText, Activity, UserPlus, Trash2, Eye, Search, Filter, Loader2 } from 'lucide-react';
...
export default function DashboardAdmin() {
  ...
  const handleCreateStaff = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Ensure payload matches your backend schema expectations
      await createUser({
        ...form,
        assignedWard: form.assignedWard.toString(), // Converted to string to prevent type errors
        role: 'ward'
      });
      ...
      console.error(err);
      // Explicitly show the backend error message so you know exactly what failed
      alert(err.response?.data?.message || err.message || 'Failed to create staff. Check your backend validations.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const totalUsers = staffList.length + 1; // Base calculation
              ...
              <table className="w-full text-left border-collapse">
                ...
                <tbody className="text-sm">
                  {filteredApplications.slice(0, 15).map((app) => {
                    const isApproved = ['approved', 'ready_for_collection'].includes(app.status);
                      ...
                      <tr key={app._id} className="border-b border-slate-50 last:border-0 hover:bg-slate-50/50 transition-colors">
                        ...
                        <td className="py-4 px-4 whitespace-nowrap">
                          <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wide border ${
                            ...
                            isReview ? 'text-yellow-700 bg-yellow-50 border-yellow-200' :
